...
/// adv diff SETUP
    ...
    float vx = -0.5f;  // velocity in x direction (positive = right)
    float vy = 0.2f;  // veloity in y direction (positive = up)
    ...
    float dt_diffusion = (r_diff * dx * dx) / Dx;
    float dt_advection_x = (vx != 0.0f) ? 0.8f * dx / fabs(vx) : 1e10f;
    float dt_advection_y = (vy != 0.0f) ? 0.8f * dy / fabs(vy) : 1e10f;

    float dt = fmin(fmin(dt_diffusion, dt_advection_x), dt_advection_y);

    float rx = Dx * dt / (dx * dx);
    float ry = Dy * dt / (dy * dy);
    float cx = vx * dt / dx;  // courange in x
    float cy = vy * dt / dy;  // courant in y

    std::cerr << "=== PARAMETER CHECK ===" << std::endl;
    std::cerr << "dx = " << dx << ", dy = " << dy << std::endl;
    std::cerr << "dt from diffusion: " << dt_diffusion << std::endl;
    std::cerr << "dt from advection x: " << dt_advection_x << std::endl;
    std::cerr << "dt from advection y: " << dt_advection_y << std::endl;
    std::cerr << "Using dt: " << dt << std::endl;
    std::cerr << "rx = " << rx << ", ry = " << ry << std::endl;
    std::cerr << "rx + ry = " << (rx + ry)  << std::endl;
...
/// END OF SETUP
    ...
    if (frameCounter % frameSkip == 0) {
        ...
        for (int step = 0; step < stepsPerFrame; step++) {
            // UPDATE INTERIOR POINTS WITH ADVECTION + DIFFUSION
            for (int i = 1; i < nx-1; ++i) {
                for (int k = 1; k < ny-1; ++k) {
                    
                    // Advection in x (upwind based on velocity sign)
                    float advection_x;
                    if (vx > 0) {
                        advection_x = -cx * (u_past[i][k] - u_past[i-1][k]);
                    } else {
                        advection_x = -cx * (u_past[i+1][k] - u_past[i][k]);
                    }
                    
                    // Advection in y (upwind based on velocity sign)
                    float advection_y;
                    if (vy > 0) {
                        advection_y = -cy * (u_past[i][k] - u_past[i][k-1]);
                    } else {
                        advection_y = -cy * (u_past[i][k+1] - u_past[i][k]);
                    }
                    
                    // Diffusion (centered difference - same as before)
                    float diffusion_x = rx * (u_past[i+1][k] - 2*u_past[i][k] + u_past[i-1][k]);
                    float diffusion_y = ry * (u_past[i][k+1] - 2*u_past[i][k] + u_past[i][k-1]);
                    
                    // COMBINE: advection + diffusion
                    u_current[i][k] = u_past[i][k] + advection_x + advection_y + 
                                      diffusion_x + diffusion_y;
                }
            }

            for (int i = 0; i < nx; ++i) {
                // Bottom (inflow if vy < 0, outflow if vy > 0)
                if (vy > 0) {
                    u_current[i][0] = u_current[i][1];  // Outflow: extrapolate
                } else {
                    u_current[i][0] = 0.0f;  // Inflow: Dirichlet
                }
                
                // Top (outflow if vy > 0, inflow if vy < 0)
                if (vy > 0) {
                    u_current[i][ny-1] = u_current[i][ny-2];  // Outflow: extrapolate
                } else {
                    u_current[i][ny-1] = 0.0f;  // Inflow: Dirichlet
                }
            }

            for (int k = 0; k < ny; ++k) {
                // Left (inflow if vx < 0, outflow if vx > 0)
                if (vx > 0) {
                    u_current[0][k] = u_current[1][k];  // Outflow: extrapolate
                } else {
                    u_current[0][k] = 0.0f;  // Inflow: Dirichlet
                }
                
                // Right (outflow if vx > 0, inflow if vx < 0)
                if (vx > 0) {
                    u_current[nx-1][k] = u_current[nx-2][k];  // Outflow: extrapolate
                } else {
                    u_current[nx-1][k] = 0.0f;  // Inflow: Dirichlet
                }
            }
            ...
            if (step == 0 && frameCounter % 10 == 0) {
                float max_val = 0.0f;
                for (int i = 0; i < nx; ++i) {
                    for (int k = 0; k < ny; ++k) {
                        max_val = fmax(max_val, fabs(u_current[i][k]));
                    }
                }
                std::cerr << "Frame " << frameCounter << ", max |u| = " << max_val << std::endl;
            }
        }
    }
